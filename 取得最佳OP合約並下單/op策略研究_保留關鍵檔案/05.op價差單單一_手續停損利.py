...
tempDate_call,tempDate_sell=0,0
class MyPosition:
    def __init__(
        self,
        openPrice=0,
...
spreadNumVar=100
OI_switch=8000
price_switch_up=20 #買入價格的上限
price_switch_down=price_switch_up*0.7 #買入價格的下限
tempTaiex_1=df_OPoi['TAIEX_close'].iloc[0:2]
charge=2
for i in range(len(df_OPoi['date'])):
    try:
        if position1.on_off ==False and df_OPoi['open_interest'].iloc[i] >= OI_switch and df_OPoi['OP_close'].iloc[i]< price_switch_up and df_OPoi['OP_close'].iloc[i]> price_switch_down and df_OPoi['call_put'].iloc[i]==call_put_switch: # and df_OPoi['contract_date'].iloc[i] != ('202003W4' or '202101W2'):
            #建倉
            position1 = MyPosition(
                on_off=True,
            ...
            try:
                ...
                if position1.possibleEarn>spreadNumVar+5:
                    position1=MyPosition()
    ...
    if position1.on_off ==True  and df_OPoi['call_put'].iloc[i] == position1.call_put :
        ...
        if df_OPoi['strike_price'].iloc[i] == position1.strikePrice_another:
            ...
            temp_i_buy=i
            tempDate_call=df_OPoi['OP_close'].iloc[i]
        ...
        if df_OPoi['strike_price'].iloc[i] == position1.strikePrice:
            ...
            tempDate=df_OPoi['date'].iloc[i]
            # OK這個日期已經取完報價了
            tempDate_sell=df_OPoi['OP_close'].iloc[i]
            tempTaiex_1=df_OPoi['TAIEX_close'].iloc[i]
            #更新一下加權指數價格，之後畫圖時要用
            temp_i_sell=i

    if position1.on_off ==True and (df_OPoi['current_contract'].iloc[i] != position1.openContract) or (tempDate_call==tempDate_sell and (equity1.tempData_2-equity1.tempData_1<0 or equity1.tempData_2-equity1.tempData_1>spreadNumVar)) :
        ...
        if position1.openDate==equity1.equityDate[-1]:
            ...
            continue
        equity1.totalEquity.append(equity1.totalEquity[-1] - equity1.equityDelta[-1] - charge)
        #把計算完成的價差變化放進淨值

        equity1.tempData_list.append([position1.openContract,0,0,0,0,0,0,0])
        equity1.tempData_list[-1][1]=equity1.equityDate[-1]
        equity1.tempData_list[-1][2]=equity1.tempData_2
        equity1.tempData_list[-1][3]=equity1.tempData_1
        equity1.tempData_list[-1][4]=position1.strikePrice
        equity1.tempData_list[-1][5]=position1.strikePrice_another
...
# tradeHistory = tradeHistory.tail(-2)
# tradeHistory.to_csv('04.opbcTEMP.csv')
# tradeHistory.to_parquet('00.parquet_home/02.STRAT_OPOI_tradeHistory.pqt')#,compression='brotli') 
...
# print(tradeHistory.iloc[2])
# table = pa.Table.from_pandas(tradeHistory)
# pq.write_table(table,'00.parquet_home/02.STRAT_OPOI_tradeHistory.pqt')

# equity1.equityDate=datetime.fromisoformat(equity1.equityDate)
fig,ax1=plt.subplots(2,1)
plottingDate=pd.to_datetime(equity1.equityDate).values

ax1[0].plot(plottingDate,equity1.totalEquity,color='red')
ax2 = ax1[0].twinx()
ax2.plot(plottingDate,equity1.taiexEquity)
ax1[0].grid(color = 'grey', linestyle = '--', linewidth = 1,axis='both')
titleTemp=''.join([str(spreadNumVar),' spread sell ',call_put_switch,' at ',str(OI_switch),' lot between ',str(price_switch_up),'and ',str(price_switch_down)])
ax2.set_title(titleTemp)

# ax1[1].ylim(top = min(tradeHistory['OP_close']))
ax1[1].plot(plottingDate,equity1.totalEquity,color='red')
...
with open("05.op回測明細.json", "w") as outfile:
